...
export const Route = createFileRoute("/recruiter/chats/$id")({
  component: RChatThreadView,
});

function RChatThreadView() {
  const { id } = useParams({ from: "/recruiter/chats/$id" });
  ...
  const send = () => {
    ...
    setText("");
    // mock reply from worker
    setTimeout(() => appendMessage(chat.id, "जी ज़रूर मैडम/सर, मैं समय पर आ जाऊँगा। 🙏", "them"), 900);
  };
  ...
  return (
    <div className="flex h-[calc(100dvh-72px)] flex-col bg-[oklch(0.96_0.02_85)]">
      <header className="flex items-center gap-3 border-b border-border bg-surface px-4 py-3 shadow-soft">
        <button onClick={() => nav({ to: "/recruiter/chats" })} className="text-xl">←</button>
